...
class TransE(torch.nn.Module):
    def __init__(self, ent_num: int, rel_num: int, dataloader: Dataloader, dim: int = 64, l1: bool = True,
                 margin: float = 1, learning_rate: float = 1e-3, weight_decay: float = 1e-3, device_index: int = 0):
        super().__init__()
...
class KGRS:
    def __init__(self, train_pos: np.array, train_neg: np.array, kg_lines: List[str]):
        ...
        config = {"batch_size": 64, "eval_batch_size": 1024, "neg_rate": 2, "emb_dim": 256, "l1": True, "margin": 20,
                  "learning_rate": 1e-4, "weight_decay": 1e-3, "epoch_num": 40}
